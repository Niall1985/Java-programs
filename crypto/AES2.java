import java.util.Scanner;

public class AESKeyGeneration {

    static final int[] S_BOX = {
        
    };

    static final int[] RCON = {
        0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
    };

    // MixColumns matrix
    static final byte[][] MIX_COLUMNS_MATRIX = {
        {(byte) 0x02, (byte) 0x03, (byte) 0x01, (byte) 0x01},
        {(byte) 0x01, (byte) 0x02, (byte) 0x03, (byte) 0x01},
        {(byte) 0x01, (byte) 0x01, (byte) 0x02, (byte) 0x03},
        {(byte) 0x03, (byte) 0x01, (byte) 0x01, (byte) 0x02}
    };

    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);


        String key = sc.nextLine();

        byte[] round0 = new byte[16];
        byte[] inputBytes = key.getBytes();

        int len = Math.min(inputBytes.length, 16);
        System.arraycopy(inputBytes, 0, round0, 0, len);

        System.out.println("\nRound 0 Key (HEX):");
        printHex(round0);

        byte[] round1 = generateRound1Key(round0);

        System.out.println("\nRound 1 Key (HEX):");
        printHex(round1);

     
        String plaintext = sc.nextLine();

        byte[] plainBytes = new byte[16];
        byte[] ptInputBytes = plaintext.getBytes();
        int ptLen = Math.min(ptInputBytes.length, 16);
        System.arraycopy(ptInputBytes, 0, plainBytes, 0, ptLen);

        System.out.println("\nPlaintext (HEX):");
        printHex(plainBytes);

        byte[][] state = bytesToState(plainBytes);

        addRoundKey(state, round0);
  
        subBytes(state);
        shiftRows(state);
        mixColumns(state);
        addRoundKey(state, round1);

        byte[] ciphertext = stateToBytes(state);

        System.out.println("\nRound 1 Ciphertext (HEX):");
        printHex(ciphertext);

        sc.close();
    }

    static byte[] generateRound1Key(byte[] key) {
        byte[] round1 = new byte[16];

        byte[] temp = {
            key[13], key[14], key[15], key[12]
        };

        for (int i = 0; i < 4; i++) {
            temp[i] = (byte) S_BOX[temp[i] & 0xFF];
        }

        temp[0] ^= RCON[0];

        for (int i = 0; i < 4; i++) {
            round1[i] = (byte) (key[i] ^ temp[i]);
        }

        for (int i = 4; i < 16; i++) {
            round1[i] = (byte) (key[i] ^ round1[i - 4]);
        }

        return round1;
    }

    static byte[][] bytesToState(byte[] bytes) {
        byte[][] state = new byte[4][4];
        for (int i = 0; i < 16; i++) {
            state[i % 4][i / 4] = bytes[i];
        }
        return state;
    }

    static byte[] stateToBytes(byte[][] state) {
        byte[] bytes = new byte[16];
        for (int i = 0; i < 16; i++) {
            bytes[i] = state[i % 4][i / 4];
        }
        return bytes;
    }

    static void subBytes(byte[][] state) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                state[i][j] = (byte) S_BOX[state[i][j] & 0xFF];
            }
        }
    }

    static void shiftRows(byte[][] state) {

        byte temp = state[1][0];
        state[1][0] = state[1][1];
        state[1][1] = state[1][2];
        state[1][2] = state[1][3];
        state[1][3] = temp;

 
        temp = state[2][0];
        state[2][0] = state[2][2];
        state[2][2] = temp;
        temp = state[2][1];
        state[2][1] = state[2][3];
        state[2][3] = temp;

  
        temp = state[3][0];
        state[3][0] = state[3][3];
        state[3][3] = state[3][2];
        state[3][2] = state[3][1];
        state[3][1] = temp;
    }

  
    static void mixColumns(byte[][] state) {
        for (int c = 0; c < 4; c++) {
            byte[] col = new byte[4];
            for (int r = 0; r < 4; r++) {
                col[r] = state[r][c];
            }
            byte[] newCol = new byte[4];
            for (int r = 0; r < 4; r++) {
                newCol[r] = (byte) (gfMul(MIX_COLUMNS_MATRIX[r][0], col[0]) ^
                                     gfMul(MIX_COLUMNS_MATRIX[r][1], col[1]) ^
                                     gfMul(MIX_COLUMNS_MATRIX[r][2], col[2]) ^
                                     gfMul(MIX_COLUMNS_MATRIX[r][3], col[3]));
            }
            for (int r = 0; r < 4; r++) {
                state[r][c] = newCol[r];
            }
        }
    }


    static byte gfMul(byte a, byte b) {
        byte p = 0;
        byte hiBitSet;
        for (int i = 0; i < 8; i++) {
            if ((b & 1) != 0) {
                p ^= a;
            }
            hiBitSet = (byte) (a & 0x80);
            a <<= 1;
            if (hiBitSet != 0) {
                a ^= 0x1B; 
            }
            b >>= 1;
        }
        return p;
    }


    static void addRoundKey(byte[][] state, byte[] key) {
        for (int i = 0; i < 16; i++) {
            int row = i % 4;
            int col = i / 4;
            state[row][col] ^= key[i];
        }
    }

    static void printHex(byte[] data) {
        for (byte b : data) {
            System.out.printf("%02X ", b);
        }
        System.out.println();
    }
}
