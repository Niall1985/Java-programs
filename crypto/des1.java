...
public class Main {
    ...
    static String binaryToHex(String bin) {
        ...
        for (int i = 0; i < bin.length(); i += 4)
            hex.append(Integer.toHexString(
                Integer.parseInt(bin.substring(i, i+4),2)
            ).toUpperCase());
        return hex.toString();
    }
    ...
    static List<String> generateKeys(String key64) {
    ...
    static String feistel(String R, String key) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        String rowBits = "" + block.charAt(0) + block.charAt(5);
        int row = Integer.parseInt(rowBits, 2);

        // Column = middle 4 bits
        String colBits = block.substring(1, 5);
        ...
        sboxResult.append(bin);
    }

    // Step 4: P Permutation
    return permute(sboxResult.toString(), P);
}

    // ---------------- BLOCK ENCRYPT ----------------

    static String encryptBlock(String block64, List<String> keys) {
        ...
        for (int i = 0; i < 16; i++) {
            String newR = xor(L, feistel(R, keys.get(i)));
            L = R;
            R = newR;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter key (8 chars recommended):");
        String keyText = sc.nextLine();

        // Ensure key is 8 bytes
        keyText = String.format("%-8s", keyText).substring(0,8);

        String keyBinary = textToBinary(keyText);
        List<String> keys = generateKeys(keyBinary);

        // Convert plaintext to binary
        String binPlain = textToBinary(plaintext);

        // Padding
        while (binPlain.length() % 64 != 0)
            binPlain += "00000000";

        StringBuilder cipherBinary = new StringBuilder();

        for (int i = 0; i < binPlain.length(); i += 64) {
            String block = binPlain.substring(i, i+64);
            cipherBinary.append(encryptBlock(block, keys));
        }

        System.out.println("\nCipher (HEX):");
        System.out.println(binaryToHex(cipherBinary.toString()));
